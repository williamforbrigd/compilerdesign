...
let string_of_stream (stream : elt list) : string =
  stream |> List.fold_left (fun init el ->
    init ^ begin match el with
    |L lbl -> " lbl: " ^ lbl
    |I(uid, ins) -> " uid: " ^ uid ^ " ins : " ^ Llutil.string_of_insn ins
    |T term -> " term: "  ^ Llutil.string_of_terminator term
    |G(gid, gdecl)  -> " gid: " ^ gid ^ " gdecl: " ^ Llutil.string_of_gdecl gdecl
    |E(uid, ins) -> " uid: " ^ uid ^ " ins: " ^ Llutil.string_of_insn ins
    end ^ "\n"
    ) ""
...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  ...
  | Ast.CStruct (id, l) ->
    ...
    let ind_code = List.fold_left (fun field_code field ->
      let field_ty, field_op, stream = cmp_exp tc c (snd field) in
      let opt = TypeCtxt.index_of_field_opt id (fst field) tc in
      begin match opt with
      |Some i ->
        let ind = gensym "struct_field" in
        lift [
          id, Gep(struct_ty, struct_op, [Const 0L; Const 1L; Const (Int64.of_int i)])
          ; "", Store(field_ty, field_op, Id ind)
        ]
      |None -> failwith "Could not find the field"
      end
    ) [] l in
    let res = alloc_code >@ ind_code in
...
and cmp_exp_lhs (tc : TypeCtxt.t) (c:Ctxt.t) (e:exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Proj (e, i) ->
    ...
    |Id id -> 
      let ty, intt = TypeCtxt.lookup_field_name id i tc in
      let ans_ty = cmp_ty tc ty in
      let ans_id = gensym "proj" in
      struct_ty, Id (ans_id), struct_code >:: I(ans_id, Load(Ptr ans_ty, struct_op))

    |_ -> failwith "struct has to be an id"
    end
