...
let print_stream (s:stream) : unit = 
  s |>
  List.fold_left (fun init el -> 
    let str = begin match el with
      |L lbl -> lbl
      |I(uid, insn)-> " UID: " ^ uid ^ " " ^ Llutil.string_of_insn insn
      |T term -> " TERM: " ^  " " ^ Llutil.string_of_terminator term
      |G(gid, gdecl) -> " GID: " ^ gid ^ " " ^ Llutil.string_of_gdecl gdecl
      |E(uid, insn) -> " UID: " ^ uid ^ " " ^ Llutil.string_of_insn insn
    end
    in init ^ str
    ) "" 
  |> print_endline;;
...
let first_of_sec(t: ('a * ('b * 'c))) : 'b = 
  begin match t with
    | (_,(x,_))->x
  end

let sec_of_sec(t: ('a * ('b * 'c))) : 'c = 
  begin match t with
    | (_,(_,x))->x
  end

let print_ctxt (c: Ctxt.t) : unit = 
  c |> 
  List.fold_left (fun init el -> init ^ "AST ID: " ^ (fst el) ^ " TYPE: " ^ Llutil.string_of_ty (first_of_sec el) ^ " OPERAND: " ^ Llutil.string_of_operand (sec_of_sec el) ^ "\n") "" 
  |> print_endline;;

let rec cmp_exp (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  (Ll.Void, Ll.Null, [L "NULL"])
  (* begin match exp.elt with *)
    (* | CNull rty -> let c = cmp_rty rty in (Ll.Void, Ll.Null, T()) *)
    (* | CBool b -> (I1, ) *)
    (* | (CBool _|CInt _|CStr _|CArr (_, _)|Id _) -> *)
  (* end *)
...
let rec cmp_stmt (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  print_endline (Llutil.string_of_ty rt);
  ...
  begin match stmt.elt with
    |Ret(opt)-> 
      begin match opt with
        |Some(exp)->
          begin match exp.elt with
            |CInt i -> c, [T(Ret(I64, Some(Const(i))))]
          end
      end
  end
...
let cmp_global_ctxt (c:Ctxt.t) (p:Ast.prog) : Ctxt.t =
  List.fold_left (fun c -> function
    |Ast.Gvdecl {elt={name; init}} -> 
      let gv = begin match init.elt with
        |CNull rty -> cmp_rty rty
        |CBool b -> I1
        |CInt i -> I64
        |CStr s -> Ptr(I8)
      end
      in Ctxt.add c name (gv, Gid(name))
    | _ -> c
  ) c p
...
let cmp_fdecl (c:Ctxt.t) (f:Ast.fdecl node) : Ll.fdecl * (Ll.gid * Ll.gdecl) list =
  let elt = f.elt in
  let return_type = cmp_ret_ty elt.frtyp in

  let c, stream = cmp_block c (cmp_ret_ty elt.frtyp) elt.body in
  print_endline "this is the new context";
  print_ctxt c;
  print_endline "";
  print_stream stream;

  let counter = ref 0 in
  let argtypes = List.fold_left (fun init arg -> 
    (*Don't need to do anything with the id because it is added to the instruction*)
    print_endline ("the arg is: " ^ snd arg);
    counter := !counter + 1;
    init @ [cmp_ty (fst arg)]
    ) [] elt.args in
  print_endline "the argtypes is: ";
  List.fold_left (fun init el -> init ^ " " ^ Llutil.string_of_ty el) "" argtypes |> print_endline;
  let f_ty = (argtypes, return_type) in
  let f_param = [] in
  (*For each of the function arguments, allocate stack space *)
  let alloca_insns = List.fold_left (fun init arg ->
    (*the UID of the instruction just the id in the AST*)
    let uid = string_of_int (!counter + 1) in
    let insn = Ll.Alloca (cmp_ty (fst arg)) in
    counter := !counter + 1;
    init @ [(uid, insn)]
    ) [] elt.args in
  (*store the function arguments in the corresponding alloca slot. All the function arguments are locals*)
  let alloca_counter = ref ((List.length alloca_insns) + 1) in
  let index = ref 0 in
  let store_insns = List.fold_left (fun init arg -> 
    let uid = string_of_int (!counter + 1) in 
    let arr = Array.of_list elt.args in
    let src = snd (Array.get arr !index) in
    let ty = match (snd arg) with Alloca ty -> ty in
    let insn = Ll.Store(ty, Id(src), Id(string_of_int !alloca_counter)) in
    counter := !counter + 1;
    index := !index + 1;
    alloca_counter := !alloca_counter + 1;
    init @ [(uid, insn)]
    ) [] alloca_insns in
  let insns = alloca_insns @ store_insns in
  print_endline @@ List.fold_left (fun init el -> init ^ "ID: " ^ fst el ^ " Insn: " ^ Llutil.string_of_insn (snd el) ^ "\n") "" insns;

  let term = ("", Ll.Br("")) in 
  let block = {insns; term} in
  let lbled_blocks = [] in
  let f_cfg = (block, []) in
...
let rec cmp_gexp c (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  (* TODO gs is global definitions what is the use case here??*)
  let gs = [] in
  begin match e.elt with
    |CNull r-> ((cmp_rty r, Ll.GNull), [])
    |CBool b -> ((I1, GInt(int64_of_bool b)), [])
    |CInt i -> ((I64, GInt(i)), [])
    |CStr s -> ((Ptr(I8), GString(s)), [])
    |_-> failwith "Only certain types can appear as global initializers."
  end
