...
let centimes_of : int -> int -> int =
    fun (rp:int) -> fun(fr:int) -> 
        if rp  < 0 || fr < 0 then failwith("All numbers must be non-negative")
        else rp + (100*fr)
...
let compose_pair (p:(('b -> 'c) * ('a -> 'b))) : 'a -> 'c =
  let f1 = first_of_two p in
  let f2 = second_of_two p in
  fun x->f1(f2(x));;

(* Alternatively:
let compose_pair (p:(('b -> 'c) * ('a -> 'b))) : 'a -> 'c =
  let compose_pair_aux p (x:'a) : 'c = 
    let f1 = first_of_two p in
    let f2 = second_of_two p in
    f1(f2(x))
  in (fun x->compose_pair_aux p x)
*)
...
let rec append (l1:'a list) (l2:'a list) : 'a list =
    begin match l1, l2 with
        | [],lst->lst
        | lst,[]->lst
        | (h1::tl1),_ -> h1 :: append tl1 l2 (*Only need to deconstruct l1*)
    end
  ...
  if is_sorted l then 
    begin match l with
      ...
      |h::tl -> 
          ...
          then x :: l else h :: insert x tl 
    end
  else l;; (*If the input list is not sorted*) 
...
let get_operation (e:exp) : string = 
  begin match e with
    |Add(_,_)->"+"
    |Mult(_,_)->"*"
    |_->raise Not_found
...
let rec string_of (e:exp) : string =
  begin match e with
    |Var x->x 
    |Const x->string_of_int (Int64.to_int x)
    |Add(e1,e2)|Mult(e1,e2)->
        let op : string = get_operation e in
        let e1', e2' = string_of e1, string_of e2 in
        "(" ^ e1' ^ " " ^ op ^ " " ^ e2' ^ ")"
    |Neg(e1)->"-" ^ "(" ^ string_of e1 ^ ")"
  end
...
let rec interpret (c:ctxt) (e:exp) : int64 =
  begin match e with
    ...
    |Var x->lookup x c
    |Add(e1,e2)|Mult(e1,e2)->
        let op = get_operation e in
        let e1', e2' = interpret c e1, interpret c e2 in
...
let rec compile (e:exp) : program =
  let acc : program = [] in
    begin match e with
      |Const x->(IPushC x)::acc
      |Var x->(IPushV x)::acc
      |Add(e1,e2)|Mult(e1,e2)->
          let op = get_operation e in
          let p1, p2 = compile e1, compile e2 in
          if String.equal op "+" then
            append (append p1 p2) ((IAdd)::acc)
          else
            append (append p1 p2) ((IMul)::acc)
      |Neg(e1)->append (compile e1) ((INeg)::acc)
    end
